      ...
      .then((response) => {
        ...
        if (data.status === "Success") {
          ...
          const ResultMatches = sortedMatches.map((match) => {
            if (!(match.bracket_id in bracketNumbers)) {
              const nextBracketNumber = Object.keys(bracketNumbers).length + 1;
              bracketNumbers[match.bracket_id] = nextBracketNumber;
            }

            const bracketNumber = bracketNumbers[match.bracket_id];

            return {
              id: match.id,
              name: `Round ${bracketNumber}`,
              nextMatchId: match.nextmatchid,
              tournamentRoundText: `Round ${bracketNumber}`,
